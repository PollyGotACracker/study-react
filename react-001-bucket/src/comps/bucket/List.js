...
props
Main.js => List.js
bucketList={bucketList} : 데이터 리스트
bucketDelete={bucketDelete} : 데이터 delete 함수(drilling)
bucketUpdateView={bucketUpdateView} 
    : 데이터가 갱신되었을 때 reRendering 함수(drilling)
*/
// ====================================================================
const List = (props) => {
  // const bucketList = props.bucketList; : props 요소가 하나일 때. 고전적
  const { bucketList, bucketDelete, bucketUpdateView } = props;

  /**
   * 배열 state 데이터를 화면에 표현할 때는
   * 반드시 각 item 에 UNIQUE 한 key 값이 있어야 한다.
   * React 가 List 화면에 Rendering 할 때
   * 모든 데이터를 갱신하는 것이 아니라
   * 변화가 있는 요소만 갱신하기 위해 사용하는 값
   */
